...
class ConvBlock(nn.Module):
    def __init__(self, in_ch, out_ch, stride=1, anisotropy=(True, True)):
        ...
        self.conv2 = nn.Conv3d(
            out_ch, out_ch, kernel_size=kernel_size2, stride=1, padding=padding2, bias=False,
        )
...
class NNUnetEncoder(nn.Module):
    def __init__(self, in_ch, out_ches):
        ...
        self.convblock01 = ConvBlock(
            in_ch, out_ches[0], stride=1, anisotropy=(True, True)
        )  # (1,1,6)
        self.convblock02 = ConvBlock(
            out_ches[0], out_ches[0], stride=1, anisotropy=(True, True)
        )  # (1,1,6)

        self.convblock03 = ConvBlock(
            out_ches[0], out_ches[1], stride=2, anisotropy=(True, True)
        )  # (2,2,6)
        self.convblock04 = ConvBlock(
            out_ches[1], out_ches[1], stride=1, anisotropy=(True, True)
        )  # (2,2,6)

        self.convblock05 = ConvBlock(
            out_ches[1], out_ches[2], stride=2, anisotropy=(True, False)
        )  # (4,4,6)
        self.convblock06 = ConvBlock(
            out_ches[2], out_ches[2], stride=1, anisotropy=(False, False)
        )  # (4,4,6)

        self.convblock07 = ConvBlock(
            out_ches[2], out_ches[3], stride=2, anisotropy=(False, False)
        )  # (8,8,12)
        self.convblock08 = ConvBlock(
            out_ches[3], out_ches[3], stride=1, anisotropy=(False, False)
        )  # (8,8,12)

        self.convblock09 = ConvBlock(
            out_ches[3], out_ches[4], stride=2, anisotropy=(False, False)
        )  # (16,16,24)
        self.convblock10 = ConvBlock(
            out_ches[4], out_ches[4], stride=1, anisotropy=(False, False)
        )  # (16,16,24)

        self.convblock11 = ConvBlock(
            out_ches[4], out_ches[5], stride=2, anisotropy=(False, False)
        )  # (32,32,48)
        self.convblock12 = ConvBlock(
            out_ches[5], out_ches[5], stride=1, anisotropy=(False, False)
        )  # (32,32,48)
...
class NNUnetDecoder(nn.Module):
    def __init__(self, in_ches):
        ...
        self.convblock01 = ConvBlock(
            in_ches[0], in_ches[0], anisotropy=(True, True)
        )  # (1,1,6)
        self.convblock02 = ConvBlock(
            in_ches[1] + in_ches[0], in_ches[0], anisotropy=(True, True)
        )  # (1,1,6)
        ...
        self.convblock03 = ConvBlock(
            in_ches[1], in_ches[1], anisotropy=(True, True)
        )  # (2,2,6)
        self.convblock04 = ConvBlock(
            in_ches[1] + in_ches[2], in_ches[1], anisotropy=(True, True)
        )  # (2,2,6)
        ...
        self.convblock05 = ConvBlock(
            in_ches[2], in_ches[2], anisotropy=(False, False)
        )  # (4,4,6)
        self.convblock06 = ConvBlock(
            in_ches[2] + in_ches[3], in_ches[2], anisotropy=(False, False)
        )  # (4,4,6)
        ...
        self.convblock07 = ConvBlock(
            in_ches[3], in_ches[3], anisotropy=(False, False)
        )  # (8,8,12)
        self.convblock08 = ConvBlock(
            in_ches[3] + in_ches[4], in_ches[3], anisotropy=(False, False)
        )  # (8,8,12)
        ...
        self.convblock09 = ConvBlock(
            in_ches[4], in_ches[4], anisotropy=(False, False)
        )  # (16,16,24)
        self.convblock10 = ConvBlock(
            in_ches[4] + in_ches[5], in_ches[4], anisotropy=(False, False)
        )  # (16,16,24)
        self.up5 = nn.Upsample(
            scale_factor=(2, 2, 2), mode="trilinear", align_corners=True
        )  # (16,16,24)
...
class NNUnetNPC(nn.Module):  #
    def __init__(self, cfg):
        ...
        layer_channels = np.array(cfg.layer_channels, dtype=np.int32)
